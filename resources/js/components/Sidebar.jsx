...
const roleLabel = (role) => {
    const map = { admin: 'Administrator', clinician: 'Clinician', biller: 'Biller' };
    ...
    return (
        ...
        >
            ...
            <div style={{
                ...
                {!collapsed && (
                    <CollapseButton onClick={() => onCollapse(true)}>
                        <ChevronLeft size={15} />
                    </CollapseButton>
                )}
            </div>
            ...
            <nav style={{
                ...
                {collapsed && (
                    <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 8 }}>
                        <CollapseButton onClick={() => onCollapse(false)}>
                ...
                {NAV.map((section, si) => (
                    <div key={section.group} style={{ marginBottom: collapsed ? 8 : 20 }}>
                        {/* Group label */}
                        {!collapsed ? (
                            <p style={{
                                paddingLeft: 10,
...
/* ─── Sub-components ─────────────────────────────────────────────────────────── */
function CollapseButton({ children, onClick }) {
    const [hovered, setHovered] = useState(false);
    return (
        <button
            onClick={onClick}
...
function HelpLink({ collapsed }) {
    ...
    return (
        ...
        >
            ...
            {!collapsed && (
                <span style={{ fontSize: 13.5, fontFamily: 'DM Sans, sans-serif' }}>Help</span>
            )}
        </a>
    );
}

function MenuAction({ icon, label, variant = 'default', onClick }) {
    const [hovered, setHovered] = useState(false);
    ...
    return (
        <button
            ...
            style={{
                ...
                border: 'none',
                cursor: 'pointer',
                backgroundColor: hovered ? (isRed ? 'rgba(239,68,68,0.1)' : 'rgba(255,255,255,0.06)') : 'transparent',
                color: isRed ? '#EF4444' : '#8FA3C0',
                fontSize: 13,
                fontFamily: 'DM Sans, sans-serif',
                transition: 'background-color 130ms',
            }}
